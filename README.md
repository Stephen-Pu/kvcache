...
- **Server-pushed Fetch — pending (M-6)**. The gRPC `Fetch` handler can already accept + import the engine's `dst_remote_mr_descriptor`, but the actual byte-movement requires picking between two transport-direction options: (a) engine-initiated Pull (gRPC `Fetch` becomes a scheduler-admit gate only), or (b) server-push (extend `TcpBackend` with a write-to-remote op). M-6 will pick and ship one.
